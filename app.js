...
function updateContextHint(){const code=getCode(), c=getContext(code);$('#patientCode').value=code;$('#contextDot').classList.toggle('on',!!c);$('#contextHint').textContent=!code?'Introduce 1–2 letras si quieres continuidad local.':c?`Contexto previo de ${code} cargado · ${new Date(c.updated).toLocaleDateString('es-ES')}`:`Sin contexto previo para ${code}.`}
...
function diagnosticHtml(text,rep){const explicit=extractExplicitDiagnosis(text);const n=norm(`${text} ${reportText()}`);let lines=[];
 if(explicit)lines.push(`<div class="suggestionBox"><strong>Diagnóstico consignado en el dictado</strong><br>${esc(explicit)}</div>`);
...
function groupHistory(){try{return JSON.parse(localStorage.getItem('psikiaGroupHistoryV2')||'[]')}catch{return []}}
function saveGroupHistory(){if(!currentGroupSession)return;const h=groupHistory();h.unshift({date:new Date().toISOString(),cycle:groupCycles[$('#groupCycle').value].name,session:currentGroupSession.title,feedback:$('#groupFeedback').value.trim().slice(0,600)});localStorage.setItem('psikiaGroupHistoryV2',JSON.stringify(h.slice(0,50)));renderGroupHistory();alert('Sesión guardada en el histórico local del grupo.')}
