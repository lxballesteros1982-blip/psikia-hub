...
let wakeLock=null, dictationWanted=false, restartTimer=null, draftTimer=null, sourceTrace={}, dictationGeneration=0;
let scalePhotoData='';
...
function normalizePatientRecord(code,record){
 const r=(record&&typeof record==='object')?{...record}:{};let entries=Array.isArray(r.entries)?r.entries.filter(Boolean):[];
 if(!entries.length&&(r.summary||r.diagnosis||r.treatment||r.updated))entries=[{id:`legacy-${code}-${r.updated||Date.now()}`,date:r.updated||new Date().toISOString(),type:r.type||'follow',diagnosis:r.diagnosis||'',treatment:r.treatment||'',summary:r.summary||'',note:''}];
 entries=entries.map((e,i)=>({id:e.id||`${code}-${i}-${e.date||Date.now()}`,date:e.date||r.updated||new Date().toISOString(),type:e.type||'follow',diagnosis:e.diagnosis||'',treatment:e.treatment||'',summary:e.summary||'',note:e.note||''})).sort((a,b)=>new Date(b.date)-new Date(a.date));
 const latest=entries[0]||{};return {code,entries,summary:r.summary||latest.summary||'',diagnosis:r.diagnosis||latest.diagnosis||'',treatment:r.treatment||latest.treatment||'',updated:r.updated||latest.date||'',type:r.type||latest.type||''};
}
function normalizedPatientStore(){const raw=patientStore(),out={};for(const [code,r] of Object.entries(raw))out[code]=normalizePatientRecord(code,r);return out}
...
function getContext(code=getCode()){if(!code)return null;const r=patientStore()[code];return r?normalizePatientRecord(code,r):null}
function updateContextHint(){const code=getCode(), c=getContext(code);$('#patientCode').value=code;$('#contextDot').classList.toggle('on',!!c);$('#contextHint').textContent=!code?'Código opcional A/AB para enlazar seguimientos.':c?`Historia de ${code} cargada · ${c.entries.length} ${c.entries.length===1?'entrada':'entradas'} · última ${new Date(c.updated).toLocaleDateString('es-ES')}`:`Sin contexto previo para ${code}.`}
function patientDiagnosisLabel(r){return String(r?.diagnosis||r?.entries?.[0]?.diagnosis||'').trim()||'Sin diagnóstico de presunción consignado'}
function patientSummaryLabel(e){const s=String(e?.summary||'').trim();return s||'Sin resumen clínico guardado.'}
function renderPatients(){
 const host=$('#patientsList');if(!host)return;const q=norm($('#patientSearch')?.value||'');const store=normalizedPatientStore();let rows=Object.values(store).filter(r=>!q||norm(`${r.code} ${r.diagnosis} ${r.summary} ${r.entries.map(e=>`${e.diagnosis} ${e.summary}`).join(' ')}`).includes(q)).sort((a,b)=>new Date(b.updated||0)-new Date(a.updated||0));
 if(!rows.length){host.innerHTML='<div class="emptyPatients">No hay pacientes que coincidan.</div>';return}
 host.innerHTML=rows.map(r=>{const entries=r.entries.map(e=>`<div class="timelineEntry"><div class="timelineDate">${esc(new Date(e.date).toLocaleString('es-ES',{dateStyle:'short',timeStyle:'short'}))}</div><div class="timelineTitle">${esc(labels[e.type]||e.type||'Entrada clínica')} · ${esc(e.diagnosis||'Dx no consignado')}</div><div class="timelineText">${esc(patientSummaryLabel(e))}</div>${e.note?`<details class="timelineNote"><summary>Ver nota guardada</summary><div class="detailBody">${esc(e.note)}</div></details>`:''}</div>`).join('');return `<details class="patientCard" data-patient="${esc(r.code)}"><summary><div class="patientHead"><div class="patientCodeBadge">${esc(r.code)}</div><div><div class="patientDx">${esc(patientDiagnosisLabel(r))}</div><div class="patientBrief">${esc(String(r.summary||'Sin resumen clínico.').slice(0,220))}</div><div class="patientMeta">Última entrada ${esc(new Date(r.updated).toLocaleDateString('es-ES'))}</div></div><div class="patientCount">${r.entries.length}</div></div></summary><div class="patientBody"><div class="patientActions"><button type="button" class="secondary patientNewNote" data-code="${esc(r.code)}">＋ Nueva nota</button><button type="button" class="ghost patientDelete" data-code="${esc(r.code)}">Eliminar</button></div><div class="timeline">${entries}</div></div></details>`}).join('');
 $$('.patientNewNote').forEach(b=>b.addEventListener('click',e=>{e.preventDefault();startPatientNote(b.dataset.code)}));$$('.patientDelete').forEach(b=>b.addEventListener('click',e=>{e.preventDefault();deletePatientRecord(b.dataset.code)}));
}
function deletePatientRecord(code){if(!confirm(`¿Eliminar la mini historia local de ${code}?`))return;const store=patientStore();delete store[code];savePatientStore(store);renderPatients();if(getCode()===code)updateContextHint()}
function activateTab(name){$$('nav button').forEach(x=>x.classList.toggle('active',x.dataset.tab===name));$$('.tab').forEach(t=>t.classList.toggle('active',t.id===`tab-${name}`));if(name==='patients')renderPatients()}
function startPatientNote(code){resetClinicalSession({keepPatient:false,keepType:true,status:`Nueva nota para ${code}. El dictado está listo.`});$('#patientCode').value=String(code||'').toUpperCase().slice(0,2);updateContextHint();activateTab('notes');$('#raw').focus()}
...
function inferPresumptiveDiagnosis(fields,prev){
 const explicit=String(fields?.diagnosis||'').trim();if(explicit)return explicit;const src=Object.keys(fullReport).length?fullReport:currentReport;for(const [k,v] of Object.entries(src)){if(/Juicio clínico|Diagnóstico al alta|Diagnóstico/i.test(k)&&String(v||'').trim())return String(v).trim().slice(0,500)}
 try{const d=v4Differential(lastInput);if(d?.items?.[0])return `${d.items[0].name}${d.items[0].code?` · ${d.items[0].code}`:''} (orientativo)`}catch{}
 return String(prev?.diagnosis||'').trim();
}
function saveCurrentContext(){
 const code=getCode();if(!code)return $('#saveHint').textContent='Añade un código A/AB para guardar la entrada en su mini historia.';const store=patientStore(),prev=normalizePatientRecord(code,store[code]||{}),fields=currentPersistentFields(lastInput,fullReport),now=new Date().toISOString();const diagnosis=inferPresumptiveDiagnosis(fields,prev),summary=makeSummary().slice(0,3000),treatment=fields.treatment||prev.treatment||'',note=reportText().slice(0,12000);
 const signature=norm(`${currentType}|${diagnosis}|${summary}|${note}`).slice(0,16000);const latest=prev.entries[0];const latestSig=latest?norm(`${latest.type}|${latest.diagnosis}|${latest.summary}|${latest.note}`).slice(0,16000):'';
 if(latest&&signature===latestSig){$('#saveHint').textContent=`La última entrada de ${code} ya estaba guardada.`;return}
 const entry={id:`${code}-${Date.now()}-${Math.random().toString(36).slice(2,7)}`,date:now,type:currentType,diagnosis,treatment,summary,note};const entries=[entry,...prev.entries];store[code]={code,entries,summary,diagnosis:diagnosis||prev.diagnosis||'',treatment,updated:now,type:currentType};savePatientStore(store);updateContextHint();renderPatients();$('#saveHint').textContent=`Entrada guardada en la mini historia de ${code} · ${entries.length} ${entries.length===1?'entrada':'entradas'}.`}
...
function hardStopRecognition(message='Dictado detenido.',save=true){
 dictationWanted=false;isDictating=false;dictationGeneration++;clearTimeout(restartTimer);const r=recognition;recognition=null;if(r){try{r.onresult=null;r.onerror=null;r.onend=null;r.abort()}catch{try{r.stop()}catch{}}}releaseWakeLock();if($('#dictate'))$('#dictate').textContent='🎙️ Dictar';setDictationStatus(message);if(save)saveDraftNow();
}
function stopDictation(message='Dictado detenido. El texto queda guardado.'){hardStopRecognition(message,true)}
function resetClinicalSession({keepPatient=false,keepType=true,status='Nueva nota. Campo limpio y dictado listo.'}={}){
 hardStopRecognition('Reiniciando dictado…',false);clearTimeout(draftTimer);$('#raw').value='';if(!keepPatient)$('#patientCode').value='';if(!keepType)$('#visitType').value='first';$('#resultCard').hidden=true;currentReport={};fullReport={};lastInput='';sourceTrace={};compactMode=false;if($('#saveHint'))$('#saveHint').textContent='';try{clearScalePhoto()}catch{};try{localStorage.removeItem(storageKey('draft'))}catch{};updateContextHint();setDictationStatus(status);
}
function startRecognitionLoop(){
 if(!dictationWanted||recognition)return;const SR=window.SpeechRecognition||window.webkitSpeechRecognition;if(!SR){dictationWanted=false;$('#raw').focus();$('#dictate').textContent='🎙️ Dictar';setDictationStatus('Este navegador no ofrece dictado web. Usa el micrófono del teclado.');return}
 const generation=dictationGeneration;const r=new SR();recognition=r;r.lang='es-ES';r.continuous=true;r.interimResults=false;r.maxAlternatives=1;isDictating=true;$('#dictate').textContent='⏹️ Detener dictado';acquireWakeLock();setDictationStatus('🎙️ Escuchando · nueva sesión activa · guardado automático.');
 r.onresult=e=>{if(generation!==dictationGeneration||!dictationWanted)return;let finalText='';for(let i=e.resultIndex;i<e.results.length;i++){if(e.results[i].isFinal)finalText+=e.results[i][0].transcript+' '}if(finalText)appendRecognized(finalText)};
 r.onerror=e=>{if(generation!==dictationGeneration)return;if(['not-allowed','service-not-allowed'].includes(e.error)){dictationWanted=false;$('#dictate').textContent='🎙️ Dictar';setDictationStatus('Permiso de micrófono no disponible. Revisa el permiso del sitio o usa el micrófono del teclado.')}else if(e.error==='audio-capture'){dictationWanted=false;$('#dictate').textContent='🎙️ Dictar';setDictationStatus('No se ha podido abrir el micrófono. Pulsa Dictar otra vez.')}else if(e.error!=='aborted')setDictationStatus('El dictado se interrumpió; reintentamos sin perder el texto.')};
 r.onend=()=>{if(generation!==dictationGeneration)return;if(recognition===r)recognition=null;isDictating=false;if(dictationWanted&&document.visibilityState==='visible'){restartTimer=setTimeout(()=>{if(generation===dictationGeneration)startRecognitionLoop()},250)}else{$('#dictate').textContent='🎙️ Dictar';releaseWakeLock()}};
 try{r.start()}catch{if(recognition===r)recognition=null;if(generation===dictationGeneration&&dictationWanted)restartTimer=setTimeout(startRecognitionLoop,450)}
}
function toggleDictation(){if(dictationWanted){stopDictation();return}hardStopRecognition('Preparando micrófono…',false);dictationGeneration++;dictationWanted=true;startRecognitionLoop()}
...
function v4Anchors(text){const s=String(text||'');let anchors=[];const add=(kind,re,weight=10)=>{const rx=new RegExp(re.source,re.flags.includes('g')?re.flags:re.flags+'g');let m;while((m=rx.exec(s))){anchors.push({kind,start:m.index,end:rx.lastIndex,weight,label:m[0]});if(!m[0])rx.lastIndex++}};
 ...
 add('mse',/\b(?:(?:a|en)\s+(?:nivel\s+de\s+)?(?:la\s+)?)?exploraci[oó]n(?:\s+psicopatol[oó]gica)?\b/ig,24);
 add('mse',/\b(?:el\s+)?paciente\s+(?:est[aá]\s+)?consciente\s*(?:,|y)?\s*orientad[oa]\b/ig,15);
...
function v4SplitFollow(text){
 ...
 const out={summary:'',evolution:'',mse:'',judgment:'',plan:''};if(!chosen.length){out.evolution=src;return out}out.summary=src.slice(0,chosen[0].start).trim();for(let i=0;i<chosen.length;i++){const a=chosen[i],next=chosen[i+1];let body=src.slice(a.end,next?next.start:src.length).trim();if(!body&&a.end===a.start)body=src.slice(a.start,next?next.start:src.length).trim();out[a.kind]=[out[a.kind],body].filter(Boolean).join(' ').trim()}return out;
}
...
$('#clear').addEventListener('click',()=>resetClinicalSession({keepPatient:false,keepType:true,status:'Nueva nota. Paciente y texto anteriores eliminados; dictado listo.'}));
...
$('#clearPatients').addEventListener('click',()=>{if(confirm('¿Borrar todas las mini historias locales A/AB y todas sus entradas?')){localStorage.removeItem(storageKey('patients'));updateContextHint();renderPatients();alert('Mini historias locales borradas.')}});
...
localStorage.setItem('psikiaHubRunningVersion',APP_VERSION);const __users=loadUsers();activeUserId=(__users.activeId&&__users.profiles[__users.activeId])?__users.activeId:Object.keys(__users.profiles)[0];migrateLegacyForActive();renderUserUI();restoreDraft();fillCycles();renderGroupHistory();updateContextHint();renderPatients();if(!$('#raw').value)setDictationStatus('Guardado automático activo. Durante el dictado intentaremos mantener la pantalla encendida.');
